...
/* Include Module */
include_once(dirname(__FILE__).'/../../otproductcomments.php');
/* Include Models */
include_once(dirname(__FILE__).'/../../classes/OtProductComment.php');
include_once(dirname(__FILE__).'/../../classes/OtProductCommentCriterion.php');

class OtProductCommentsDefaultModuleFrontController extends ModuleFrontController 
{
    ...
    {
        $module_instance = new OtProductComments();
        ...
        if (!$this->context->customer->id && !Configuration::get('OTPRODUCTCOMMENT_ALLOW_GUESTS')) {
        ...
        if (!count($errors)) {
            $customer_comment = OtProductComment::getByCustomer(Tools::getValue('id_product'), $id_customer, true, $id_guest);
            if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + (int)Configuration::get('OTPRODUCTCOMMENT_MINIMAL_TIME')) < time())) {
                $comment = new OtProductComment();
                ...
                foreach (Tools::getValue('criterion') as $id_product_comment_criterion => $grade) {
                    $grade_sum += $grade;
                    $product_comment_criterion = new OtProductCommentCriterion($id_product_comment_criterion);
                ...
                $result = true;
                Tools::clearCache(Context::getContext()->smarty, $this->getTemplatePath('otproductcomments_reviews.tpl'));
            } else {
                $result = false;
                $errors[] = $module_instance->l('Please wait before posting another comment', 'default').' '.Configuration::get('OTPRODUCTCOMMENT_MINIMAL_TIME').' '.$module_instance->l('seconds before posting a new comment', 'default');
            }
        } else {
            $result = false;
        }
    ...
    protected function ajaxProcessReportAbuse()
    {
        ...
        if (OtProductComment::isAlreadyReport(Tools::getValue('id_product_comment'), $this->context->cookie->id_customer)) {
            die('0');
        }

        if (OtProductComment::reportComment((int)Tools::getValue('id_product_comment'), $this->context->cookie->id_customer)) {
            die('1');
        }
    ...
    protected function ajaxProcessCommentIsUsefull()
    {
        ...
        if (OtProductComment::isAlreadyUsefulness(Tools::getValue('id_product_comment'), $this->context->cookie->id_customer)) {
            die('0');
        }

        if (OtProductComment::setCommentUsefulness((int)Tools::getValue('id_product_comment'), (bool)Tools::getValue('value'), $this->context->cookie->id_customer)) {
            die('1');
        }
