...
class OtProductComment extends ObjectModel 
{
    public $id; 
    ...
    */
    public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null)
    {
        ...
        $validate = Configuration::get('OTPRODUCTCOMMENT_MODERATE');
        $p = (int)$p;
        $n = (int)$n;
        if ($p <= 1) {
            $p = 1;
        ...
        $cache_id = 'OtProductComment::getByProduct_'.(int)$id_product.'-'.(int)$p.'-'.(int)$n.'-'.(int)$id_customer.'-'.(bool)$validate;
        if (!Cache::isStored($cache_id)) {
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                SELECT pc.`id_product_comment`,
                (SELECT count(*) FROM `'._DB_PREFIX_.'otproduct_comment_usefulness` pcu WHERE pcu.`id_product_comment` = pc.`id_product_comment` AND pcu.`usefulness` = 1) as total_useful,
                (SELECT count(*) FROM `'._DB_PREFIX_.'otproduct_comment_usefulness` pcu WHERE pcu.`id_product_comment` = pc.`id_product_comment`) as total_advice, '.
                ((int)$id_customer ? '(SELECT count(*) FROM `'._DB_PREFIX_.'otproduct_comment_usefulness` pcuc WHERE pcuc.`id_product_comment` = pc.`id_product_comment` AND pcuc.id_customer = '.(int)$id_customer.') as customer_advice, ' : '').
                ((int)$id_customer ? '(SELECT count(*) FROM `'._DB_PREFIX_.'otproduct_comment_report` pcrc WHERE pcrc.`id_product_comment` = pc.`id_product_comment` AND pcrc.id_customer = '.(int)$id_customer.') as customer_report, ' : '').'
                ...
                FROM `'._DB_PREFIX_.'otproduct_comment` pc
    ...
    */
    ...
    {
        $cache_id = 'OtProductComment::getByCustomer_'.(int)$id_product.'-'.(int)$id_customer.'-'.(bool)$get_last.'-'.(int)$id_guest;
        if (!Cache::isStored($cache_id)) {
    ...
    */
    public static function getGradeByProduct($id_product, $id_lang)
    {
        ...
        return (Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            FROM `'._DB_PREFIX_.'otproduct_comment` pc
            LEFT JOIN `'._DB_PREFIX_.'otproduct_comment_grade` pcg ON (pcg.`id_product_comment` = pc.`id_product_comment`)
            LEFT JOIN `'._DB_PREFIX_.'otproduct_comment_criterion` pcc ON (pcc.`id_product_comment_criterion` = pcg.`id_product_comment_criterion`)
            LEFT JOIN `'._DB_PREFIX_.'otproduct_comment_criterion_lang` pccl ON (pccl.`id_product_comment_criterion` = pcg.`id_product_comment_criterion`)
            WHERE pc.`id_product` = '.(int)$id_product.'
    ...
    {
        /* Get all grades */
        $grades = OtProductComment::getGradeByProduct((int)$id_product, (int)$id_lang);
        $total = OtProductComment::getGradedCommentNumber((int)$id_product);
    ...
    */
    public static function getCommentNumber($id_product)
    {
        ...
        $validate = (int)Configuration::get('OTPRODUCTCOMMENT_MODERATE');
        $cache_id = 'OtProductComment::getCommentNumber_'.(int)$id_product.'-'.$validate;
        if (!Cache::isStored($cache_id)) {
    ...
    */
    public function validate($validate = '1')
    {
        ...
        $success = (Db::getInstance()->execute('
            UPDATE `'._DB_PREFIX_.'otproduct_comment` SET
            `validate` = '.(int)$validate.'
            WHERE `id_product_comment` = '.(int)$this->id));

        Hook::exec('actionObjectOtProductCommentValidateAfter', array('object' => $this));
        return $success;
    }
    ...
    */
    ...
    {
        OtProductComment::deleteGrades($this->id);
        OtProductComment::deleteReports($this->id);
        OtProductComment::deleteUsefulness($this->id);
        parent::delete();
    }
    ...
    */
    public static function deleteGrades($id_product_comment)
    {
        ...
        return (Db::getInstance()->execute('
            DELETE FROM `'._DB_PREFIX_.'otproduct_comment_grade`
    ...
    */
    public static function deleteReports($id_product_comment)
    {
        ...
        return (Db::getInstance()->execute('
            DELETE FROM `'._DB_PREFIX_.'otproduct_comment_report`
            WHERE `id_product_comment` = '.(int)$id_product_comment));
    }
    ...
    */
    public static function deleteUsefulness($id_product_comment)
    {
        ...
        return (Db::getInstance()->execute('
            DELETE FROM `'._DB_PREFIX_.'otproduct_comment_usefulness`
            WHERE `id_product_comment` = '.(int)$id_product_comment));
    }
    ...
    */
    ...
    {
        return (Db::getInstance()->execute('
            INSERT INTO `'._DB_PREFIX_.'otproduct_comment_report` (`id_product_comment`, `id_customer`)
    ...
    */
    public static function isAlreadyReport($id_product_comment, $id_customer)
    {
        return (bool)Db::getInstance()->getValue('
            SELECT COUNT(*)
            FROM `'._DB_PREFIX_.'otproduct_comment_report`
    ...
    */
    ...
    {
        return (Db::getInstance()->execute('
            INSERT INTO `'._DB_PREFIX_.'otproduct_comment_usefulness` (`id_product_comment`, `usefulness`, `id_customer`)
    ...
    */
    public static function isAlreadyUsefulness($id_product_comment, $id_customer)
    {
        return (bool)Db::getInstance()->getValue('
            SELECT COUNT(*)
            FROM `'._DB_PREFIX_.'otproduct_comment_usefulness`
            WHERE `id_customer` = '.(int)$id_customer.'
            AND `id_product_comment` = '.(int)$id_product_comment);
    }
    ...
    */
    public static function getReportedComments()
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            FROM `'._DB_PREFIX_.'otproduct_comment_report` pcr
            LEFT JOIN `'._DB_PREFIX_.'otproduct_comment` pc
