...
class CompareProduct extends ObjectModel
{
    ...
    public static function getCompareProducts($id_compare)
    {
		...
		FROM `'._DB_PREFIX_.'otcompare` c
		LEFT JOIN `'._DB_PREFIX_.'otcompare_product` cp ON (cp.`id_compare` = c.`id_compare`)
    ...
    public static function addCompareProduct($id_compare, $id_product)
    {
        ...
        if (!$id_compare) {
			...
			INSERT INTO `'._DB_PREFIX_.'otcompare` (`id_compare`, `id_customer`) VALUES (NULL, "'.($id_customer ? $id_customer: '0').'")');
            if ($sql) {
                $id_compare = Db::getInstance()->getValue('SELECT MAX(`id_compare`) FROM `'._DB_PREFIX_.'otcompare`');
                Context::getContext()->cookie->id_compare = $id_compare;
            }
        }

        return Db::getInstance()->execute('
			INSERT IGNORE INTO `'._DB_PREFIX_.'otcompare_product` (`id_compare`, `id_product`, `date_add`, `date_upd`)
    ...
    {
        return Db::getInstance()->execute('
		DELETE cp FROM `'._DB_PREFIX_.'otcompare_product` cp, `'._DB_PREFIX_.'otcompare` c
    ...
    public static function getNumberProducts($id_compare)
    {
        return (int)(Db::getInstance()->getValue('
			...
			FROM `'._DB_PREFIX_.'otcompare_product`
			WHERE `id_compare` = '.(int)($id_compare)));
    }
    ...
    public static function cleanCompareProducts($period = null)
    {
        ...
        DELETE cp, c FROM `'._DB_PREFIX_.'otcompare_product` cp, `'._DB_PREFIX_.'otcompare` c
