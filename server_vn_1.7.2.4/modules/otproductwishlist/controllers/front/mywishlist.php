...
class OtProductWishListMyWishListModuleFrontController extends ModuleFrontController
{
    public $php_self;
    ...
    */
    public function assign()
    {
        ...
        if ($this->context->customer->isLogged()) {
            ...
            if (Tools::isSubmit('submitWishlist')) {
                ...
                if (!count($errors)) {
                    ...
                    if (OtWishList::isExistsByNameForUser($name)) {
                    ...
                    if (!count($errors)) {
                        ...
                        Mail::Send(
                            ...
                            array(
                                ...
                                '{message}' => $this->context->link->getModuleLink('OtProductWishList', 'view', array('token' => $wishlist->token))
                                ),
                        ...
                        Tools::redirect($this->context->link->getModuleLink('OtProductWishList', 'mywishlist'));
                    }
                }
            } elseif ($add) {
                OtWishList::addCardToWishlist($this->context->customer->id, Tools::getValue('id_wishlist'), $this->context->language->id);
            ...
            $this->context->smarty->assign('wishlists', OtWishList::getByIdCustomer($this->context->customer->id));
            $this->context->smarty->assign('nbProducts', OtWishList::getInfosByIdCustomer($this->context->customer->id));
        } else {
            Tools::redirect('index.php?controller=authentication&back='.urlencode($this->context->link->getModuleLink('OtProductWishList', 'mywishlist')));
        }
        ...
        $this->setTemplate('module:OtProductWishList/views/templates/front/mywishlist.tpl');
    }
    ...
    public function ajaxProcessDeleteList()
    {
        ...
        if ($default_change) {
            $array = OtWishList::getDefault($id_customer);
    ...
    public function ajaxProcessProductChangeWishlist()
    {
        ...
        $new_wishlist = new OtWishList((int)$id_new_wishlist);
        $old_wishlist = new OtWishList((int)$id_old_wishlist);
        ...
        $res = true;
        $check = (int)Db::getInstance()->getValue('SELECT quantity FROM '._DB_PREFIX_.'otwishlist_product
