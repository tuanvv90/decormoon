...
if ($context->customer->isLogged()) {
    if ($id_wishlist && OtWishList::exists($id_wishlist, $context->customer->id) === true) {
        $context->cookie->id_wishlist = (int)$id_wishlist;
    }

    if ((int)$context->cookie->id_wishlist > 0 && !OtWishList::exists($context->cookie->id_wishlist, $context->customer->id)) {
    ...
    if (($add || $delete) && empty($id_product) === false) {
        ...
        if ($add && $quantity) {
            OtWishList::addProduct($context->cookie->id_wishlist, $context->customer->id, $id_product, $id_product_attribute, $quantity);
            echo (int)Db::getInstance()->getValue('SELECT count(id_wishlist_product) FROM '._DB_PREFIX_.'otwishlist w, '._DB_PREFIX_.'otwishlist_product wp where w.id_wishlist = wp.id_wishlist and w.id_customer='.(int)$context->customer->id);
            die();
        } else if ($delete) {
            OtWishList::removeProduct($context->cookie->id_wishlist, $context->customer->id, $id_product, $id_product_attribute);
        }
    }
    $context->smarty->assign('products', OtWishList::getProductByIdCustomer($context->cookie->id_wishlist, $context->customer->id, $context->language->id, null, true));
    $context->smarty->assign('link', $context->link);
    if (Tools::file_exists_cache(_PS_THEME_DIR_.'modules/otproductwishlist/views/templates/hook/otproductwishlist-ajax.tpl')) {
        $context->smarty->display(_PS_THEME_DIR_.'modules/otproductwishlist/views/templates/hook/otproductwishlist-ajax.tpl');
    } elseif (Tools::file_exists_cache(dirname(__FILE__).'/views/templates/hook/otproductwishlist-ajax.tpl')) {
        $context->smarty->display(dirname(__FILE__).'/views/templates/hook/otproductwishlist-ajax.tpl');
    } else {
